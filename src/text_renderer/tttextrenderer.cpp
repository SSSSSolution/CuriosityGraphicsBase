...
namespace curiosity {
    ...
    TTTextRenderer::TTTextRenderer(const char *fontPath)
        : TextRenderer(fontPath), charFactory() {
        font->setFactory(charFactory);
        ...
        glBindBuffer(GL_ARRAY_BUFFER, sizeof(GLfloat)*6*4, NULL, GL_DYNAMIC_DRAW);
    ...
    void TTTextRenderer::renderText(std::wstring text, float x, float y, float scale, Vec3 color) {
        ...
        for (c = text.begin(); c != text.end(); ++c) {
            TTCharacter ch = font->getCharacter(*c);
    ...
    TTCharacter *TTCharacterFactory::createCharacter(std::string fontPath, wchar_t ch) {
        FT_Library ft;
        if (FT_Init_FreeType(&ft))
            cout << "Error:: freetype: could not init freetype lib" << endl;
        FT_Face face;
        if (FT_New_Face(ft, (fontDir()+font).c_str(), 0, &face))
